...
public class Day21Part2 {
    ...
    record Position (long x, long y, int steps) {}
    ...
    public static void main(String[] args) {
        ...
        try(BufferedReader br = Files.newBufferedReader(Paths.get(resourcesPath,"day21-puzzle-input.txt"))) {
            var initialMap = br.lines().toList();
            ...
            List.of(64,26501365).forEach(testSteps -> {
                ...
                if (testSteps > 500) {
                    ...
                    var stepsList = Stream.of(0L,1L,2L).map(i -> (width-1)/2 + (width*i)).toList();
                    var posList = stepsList.stream().map(steps -> {
                        seen = new HashSet<>();
                        var count =  recurse(garden,new Position(finalStart1.x, finalStart1.y, Math.toIntExact(steps)));
                    ...
                    var countForSmallerSets = recurse(garden,new Position(finalStart1.x, finalStart1.y, testSteps));
    ...
    private static long recurse(char[][] garden, Position from) {
        ...
        long count =  Stream.of(List.of(0L,1L), List.of(0L,-1L), List.of(1L,0L), List.of(-1L,0L))
                .map(dir -> {
                    ...
                    return switch(content) {
                        case '.','O' -> recurse(garden,new Position(posX, posY,from.steps-1));
                        case '#' -> 0L;
                        default -> 0L;
                    };
                })
